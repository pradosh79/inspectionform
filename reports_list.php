...
$result = mysqli_query($conn, "SELECT id, client, inspection_date, report_number, address, saved_at FROM inspections ORDER BY saved_at DESC LIMIT 200");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Previous Reports — A &amp; D Inspections</title>
...
<div class="rp-sheet list-sheet no-pagenum" style="border-color:#999;">
  <div class="list-title">Previous Reports</div>
  ...
  <?php else: ?>
    <?php while ($row = mysqli_fetch_assoc($result)): ?>
      <a class="report-row" href="report.php?id=<?php echo urlencode($row['id']); ?>">
        ...
        <div class="report-addr"><?php echo htmlspecialchars($row['address'], ENT_QUOTES, 'UTF-8'); ?><?php if ($row['report_number']): ?> &middot; #<?php echo htmlspecialchars($row['report_number'], ENT_QUOTES, 'UTF-8'); ?><?php endif; ?></div>
      </a>
    <?php endwhile; ?>
  <?php endif; ?>
</div>
...
<div style="text-align:center;margin-top:6px;">
  <a href="index.php" style="color:#2E7D32;font-family:Arial,sans-serif;font-size:13px;">&larr; Back to form</a>
</div>
<script>
(function(){
  function ping(){
